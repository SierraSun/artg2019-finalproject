...
migrationDataPromise.then(data=>{
  ...
  simulation
			  ...
			  .alpha(1);//set the steps of collision
...
function generateNetwork(data, year){
  ...
  let filteredData=data.filter(d=>d.year===year);
	...
	const linksData = [];
	//if present, thenmodify the existing node
	//if not presetn, create a new node
    ...
    if(!nodesData.get(od.origin_name)){//!turn fault into ture; if nodes is not exist in map then create one
       const newMode = {
				 name:od.origin_name,
				 incoming:[],
				 outgoing:[newLink],
			 ...
			 nodesData.set(od.origin_name,newMode);//set:add the new data into the map
			 newLink.source=newMode;//link the source with the origin_name of map created
		} else {
       //if the node is already present in the map
			 ...
			 newLink.source=existingNode//link the source with the origin_name of map presented
		}
        ...
        outgoingTotal: 0
			...
			nodesData.set(od.dest_name,newMode);
			newLink.target=newMode
		} else{
	   ...
	   console.log(
		 ...
		 )//nodesData.values(iteration)unique list of all the countries
		 console.log(linksData)
	//return nodesData and linksData
	return{
