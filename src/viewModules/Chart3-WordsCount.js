...
function ScatterPlot() {
  ...
  function update(data, rootDOM, value) {
    ...
    const y = d3.scaleLinear().range([innerHeight, 0]).domain([0, d3.max(data, function(d) {
      return +d[value]
    }) * 1.2])

    const color = d3.scaleSequential().domain([d3.max(data, function(d) {
      return +d[value]
    }), d3.min(data, function(d) {
      return +d[value]
    })]).interpolator(d3.interpolateRgb('#5d001e', '#e3afbc'));

    const scaleSize = d3.scaleSqrt().range([1, 6]);
    //Discover max value to set the size of circles
    scaleSize.domain([0, d3.max(data, function(d) {
      return +d[value]
    })]);
